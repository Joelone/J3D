...
J3D.ShaderUtil.setLights = function(shader, lights) {
    for (var i = 0; i < J3D.SHADER_MAX_LIGHTS; i++) {
        ...
        if (l && shader.uniforms["uLight[" + i + "].type"]) {
            var li = lights[i];
            gl.uniform1i(shader.uniforms["uLight[" + i + "].type"].location, l.light.type);
            gl.uniform3fv(shader.uniforms["uLight[" + i + "].direction"].location, l.forward().toUniform());
            gl.uniform3fv(shader.uniforms["uLight[" + i + "].color"].location, l.light.color.rgb());
            gl.uniform3fv(shader.uniforms["uLight[" + i + "].position"].location, l.worldPosition.xyz());
            gl.uniform1f(shader.uniforms["uLight[" + i + "].intensity"].location, l.light.intensity);
            gl.uniform1f(shader.uniforms["uLight[" + i + "].angleFalloff"].location, l.light.angleFalloff);
            gl.uniform1f(shader.uniforms["uLight[" + i + "].angle"].location, l.light.angle);
